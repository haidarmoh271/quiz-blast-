...
io.on('connection', socket => {
  ...
  socket.on('dots:join',({code,name,team,wantLeader})=>{
    const room=dotsRooms[code];
    if(!room) return socket.emit('error','الكود غير صحيح');
    ...
    const hasLeader=teamPlayers.some(p=>p.isLeader);
    // لو ما في قائد → أول واحد يكون قائد تلقائياً، وإلا حسب اختياره
    const isLeader = !hasLeader ? true : (wantLeader === true ? true : false);
    // لو اختار قائد وفي قائد موجود → ارفض
    if(wantLeader && hasLeader) return socket.emit('error','الفريق عنده قائد بالفعل!');
  ...
  // المضيف يغير القائد
  socket.on('dots:setLeader',({code,socketId})=>{
    const room=dotsRooms[code];
    if(!room||room.host!==socket.id) return;
    const player=room.players[socketId];
    if(!player) return;
    // أزل القيادة من الفريق الحالي
    Object.values(room.players).forEach(p=>{ if(p.team===player.team) p.isLeader=false; });
    player.isLeader=true;
    // أخبر اللاعب الجديد
    io.to(socketId).emit('dots:youAreLeader');
    const playerList=getDotsPlayerList(room);
    io.to(room.host).emit('dots:playerList',{players:playerList});
    io.to('dots_'+code).emit('dots:playerList',{players:playerList});
  });
