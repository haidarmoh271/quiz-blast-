...
function nextQuestion(code) {
  ...
  io.to('display:' + code).emit('display:question', {
    index: room.currentQ, total,
    question: q.question, answers: q.answers,
    correct: q.correct, time: q.time, image: q.image || null,
  });
...
function showResults(code) {
  ...
  const isLastQ = room.currentQ + 1 >= room.quiz.questions.length;
  const adInfo = q.showAdAfter && q.adVideoUrl ? { videoUrl: q.adVideoUrl, text: q.adText||'' } : null;
  io.to('display:' + code).emit('display:results', {
