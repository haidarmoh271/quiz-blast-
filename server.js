...
app.post('/ai-recent', async (req, res) => {
  ...
  if(!apiKey) return res.status(400).json({ error: 'ANTHROPIC_API_KEY غير مضبوط' });
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `ابحث عن أحدث الأخبار والأحداث عن "${topic}" ثم أنشئ ${count} سؤال اختيار من متعدد باللغة العربية عن أحداث حديثة ومستجدات حقيقية. مستوى الصعوبة: ${difficulty}.
...
[{"question":"...","answers":["...","...","...","..."],"correct":0}]`
    ...
    // استخرج النص من آخر content block
    const texts = (data.content||[]).filter(b=>b.type==='text').map(b=>b.text);
    const text = texts.join('');
    const match = text.match(/\[[\s\S]*\]/);
    if(!match) return res.status(500).json({ error: 'تعذر تحليل الأسئلة' });
...
function nextQuestion(code) {
  ...
  // أرسل للاعبين (بدون shuffle — الإجابات أحرف فقط A/B/C/D)
...
io.on('connection', socket => {
  ...
  socket.on('player:join', ({ code, name, team }) => {
    ...
    room.players[socket.id] = { name, team: team||'', score: 0, answers: [] };
    socket.join(code);
  ...
  socket.on('player:answer', ({ code, answerIndex, timeLeft }) => {
    ...
    // أول 5 يجاوبون (صح أو غلط) يحصلون بونص 20
    if (!room.answerOrder) room.answerOrder = {};
    if (!room.answerOrder[room.currentQ]) room.answerOrder[room.currentQ] = 0;
    const answerRank = ++room.answerOrder[room.currentQ];
    const speedBonus = answerRank <= 5 ? 20 : 0;
    const correctPoints = correct ? 50 : 0;
    const points = correctPoints + speedBonus;
    player.score += points;
    player.answers[room.currentQ] = { answerIndex: realIndex, correct, points, correctPoints, speedBonus };
